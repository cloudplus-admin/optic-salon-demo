...
function branchesView(){const branches=[['Медицинский центр Юнусабад','Ташкент','Медцентр + мастерская','18 сотрудников','84,6 млн сум','Работает'],['Магазин Чиланзар','Ташкент','Розничный магазин','9 сотрудников','46,2 млн сум','Работает'],['Медицинский центр Самарканд','Самарканд','Медцентр + магазин','14 сотрудников','57,9 млн сум','Работает'],['Центральный склад','Ташкент','Склад и логистика','11 сотрудников','246,85 млн сум','Работает']];return sectionShell([['Подразделения','4','в 2 городах'],['Сотрудники','52','11 специальностей'],['Кассы','7','6 открыты'],['Склады','7','246,85 млн сум']],`<div class="branch-grid">${branches.map((r,i)=>`<article class="card branch-card"><header><span class="branch-icon">${['✚','▣','✚','▦'][i]}</span><span class="status success">● ${r[5]}</span></header><h3>${r[0]}</h3><p>${r[1]} · ${r[2]}</p><div><span><small>Команда</small><b>${r[3]}</b></span><span><small>${i===3?'Запас':'Выручка'}</small><b>${r[4]}</b></span></div><button class="secondary" data-demo-action="Панель филиала открыта">Открыть подразделение</button></article>`).join('')}</div>`)}
