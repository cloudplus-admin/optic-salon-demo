...
function settingsView(){
  ...
  return `<div class="settings-layout"><section class="card settings-card"><span class="settings-icon">◎</span><div><span class="eyebrow">Интерфейс</span><h2>Язык и оформление</h2><p>Изменения применяются сразу и сохраняются в браузере.</p></div><div class="settings-fields"><label>Язык интерфейса<select data-setting="language"><option value="ru" ${p.language==='ru'?'selected':''}>Русский</option><option value="uz" ${p.language==='uz'?'selected':''}>O‘zbekcha</option><option value="en" ${p.language==='en'?'selected':''}>English</option></select></label><label>Цветовая тема<select data-setting="theme"><option value="light" ${p.theme!=='dark'?'selected':''}>Светлая</option><option value="dark" ${p.theme==='dark'?'selected':''}>Тёмная</option></select></label></div><div class="theme-preview"><button data-theme-choice="light" title="Светлая"></button><button data-theme-choice="dark" title="Тёмная"></button></div></section><section class="card settings-card"><span class="settings-icon">◈</span><div><span class="eyebrow">Рабочее место</span><h2>Поведение системы</h2><p>Настройте уведомления и безопасность демо.</p></div><div class="settings-switches">${['Двухфакторная аутентификация','Уведомления о низких остатках','SMS при готовности заказа','Автоматическое резервное копирование'].map((x,i)=>`<label class="switch">${x}<input type="checkbox" ${i!==0?'checked':''}></label>`).join('')}</div></section></div><button class="primary" data-save-settings style="margin-top:18px">Сохранить настройки</button>`;
}
...
function inventoryView(){return sectionShell([['Учётная стоимость','246,85 млн сум','оборотный капитал'],['Позиций','3 842','7 складов'],['Класс A','68% стоимости','312 позиций'],['Расхождения','4 позиции','−1,26 млн сум']],`<div class="inventory-layout"><section class="card audit-card"><div class="card-title"><div><h2>Инвентаризация INV-26-0731</h2><p>Центральный склад · отвечает Сергей Ли</p></div><span class="status info">62% пересчитано</span></div><div class="big-progress"><i style="width:62%"></i></div><div class="audit-stats"><span><small>Проверено</small><b>1 946</b></span><span><small>Совпало</small><b>1 942</b></span><span><small>Излишки</small><b>1</b></span><span><small>Недостача</small><b>3</b></span></div><button class="primary" data-demo-action="Сканер пересчёта открыт">Продолжить пересчёт</button></section><section class="card abc-card"><div class="card-title"><div><h2>ABC-анализ запасов</h2><p>По стоимости и оборачиваемости</p></div></div>${[['A','167,8 млн','68%','#3478f6'],['B','51,9 млн','21%','#8b6ce5'],['C','27,1 млн','11%','#d99a31']].map(x=>`<div class="abc-row"><b style="color:${x[3]}">${x[0]}</b><div><strong>${x[1]}</strong><span><i style="width:${x[2]};background:${x[3]}"></i></span></div><small>${x[2]}</small></div>`).join('')}</section></div>`)}
...
function installmentsView(){const rows=[['INS-1042','Шахноза Каримова','Протез бедра BK-04','48 млн сум','19,2 млн сум','05.08.2026','По графику'],['INS-1038','Марат Ахметов','Phonak Audéo L70-R','6,84 млн сум','2,28 млн сум','28.07.2026','Просрочен'],['INS-1029','Алексей Морозов','Кислородный концентратор','12,5 млн сум','7,5 млн сум','12.08.2026','По графику']];return sectionShell([['Активные договоры','46','92 клиента'],['Остаток рассрочек','186,4 млн сум','к получению'],['Платежи в августе','38,7 млн сум','54 платежа'],['Просрочено','11,2 млн сум','6 платежей']],`<section class="card"><div class="table-wrap"><table><thead><tr><th>Договор</th><th>Клиент</th><th>Изделие</th><th>Сумма</th><th>Остаток</th><th>Следующий платёж</th><th>Статус</th><th></th></tr></thead><tbody>${rows.map(r=>`<tr><td><b>${r[0]}</b></td><td>${r[1]}</td><td>${r[2]}</td><td>${r[3]}</td><td><b>${r[4]}</b></td><td>${r[5]}</td><td><span class="status ${r[6]==='Просрочен'?'danger':'success'}">${r[6]}</span></td><td><button class="table-action" data-demo-action="График платежей открыт">График</button></td></tr>`).join('')}</tbody></table></div></section>`)}
...
function branchesView(){const totals=NETWORK_BRANCHES.reduce((x,b)=>({staff:x.staff+b.staff,cash:x.cash+b.cashDesks,warehouses:x.warehouses+b.warehouses}),{staff:0,cash:0,warehouses:0});return sectionShell([['Подразделения',NETWORK_BRANCHES.length,'в Ташкенте и Самарканде'],['Сотрудники',totals.staff,'по всей сети'],['Кассы',totals.cash,'рабочих мест'],['Складские зоны',totals.warehouses,'в подразделениях']],`<div class="branch-grid">${NETWORK_BRANCHES.map((b,i)=>`<article class="card branch-card" data-branch-id="${b.id}"><header><span class="branch-icon">${['✚','▣','✚','▦'][i]}</span><span class="status success">● ${escapeHtml(b.status)}</span></header><h3>${escapeHtml(b.name)}</h3><p>${escapeHtml(b.city)} · ${escapeHtml(b.type)}</p><div><span><small>Команда</small><b>${b.staff} сотрудников</b></span><span><small>${b.id==='warehouse'?'Стоимость остатков':'Выручка за месяц'}</small><b>${escapeHtml(b.id==='warehouse'?b.stockValue:b.revenue)}</b></span></div><button class="secondary" data-open-branch="${b.id}">Открыть подразделение</button></article>`).join('')}</div>`)}
...
function productionBoard(){
  ...
  return `<section class="card workshop-command"><div><span class="eyebrow">Мастерская и индивидуальные работы</span><h2>${active.length} активных производственных заказов</h2><p>Вся информация собрана в карточках; этапы больше не разделены на колонки.</p></div><div class="workshop-stats"><span><b>${active.filter(o=>o.priority==='Срочный'||o.status==='Требует обеспечения').length}</b> требуют внимания</span><span><b>${Object.keys(workload).length}</b> исполнителей</span><span><b>94%</b> в срок</span></div></section><div class="workshop-toolbar card"><label class="search">⌕ <input data-work-search placeholder="Заказ, клиент, изделие, мастер или этап"></label><select data-work-filter><option>Все направления</option><option>Оптика</option><option>Слух</option><option>Протезирование</option><option>Ортопедия</option><option>Медтехника</option></select><button class="secondary" data-demo-action="Фильтр просроченных работ применён">Требуют внимания</button></div><section class="workshop-grid">${active.map((o,i)=>workshopCard(o,i)).join('')}</section>`;
}
...
function advanceWorkshop(o){
  ...
  if(current==='Контроль качества'){
    if(!confirm('Подтвердите обязательный ОТК: комплектность, параметры изделия, внешний вид и безопасность проверены.'))return;
  ...
  if(next==='Выдано'){
    ...
    if(!confirm('Подтвердить фактическую выдачу заказа клиенту?'))return;
  }
  const comment=prompt('Комментарий к переходу (необязательно):','');if(comment===null)return;
...
function returnWorkshop(o){const route=workshopRoute(o),index=workshopIndex(o);if(index<=0){notify('Это первый этап маршрута');return}const reason=prompt('Укажите обязательную причину возврата на предыдущий этап:','');if(!reason?.trim()){notify('Возврат без причины запрещён');return}const from=route[index],to=route[index-1];workshopEvent(o,from,to,reason.trim(),'Возврат');o.stage=to;o.progress=workshopProgress(o);o.status='В работе';if(from==='Готов к выдаче'||from==='Контроль качества'){o.quality='Требуется повторный ОТК';delete o.qualityChecks}save('orders',state.orders);syncDashboard();renderModule();notify(`Заказ №${o.id} возвращён: ${to}`)}
...
function openEditor(type,item=null){
  ...
  $('#entityFields').innerHTML=schema.fields.map(([name,label,kind,opts])=>{
    ...
    if(kind==='linked'){const source=opts==='clients'?state.clients:opts==='orders'?state.orders:state.employees,display=x=>opts==='clients'?`${x.name} · ${x.phone}`:opts==='orders'?`№${x.id} · ${x.client} · ${x.product||x.direction}`:`${x.name} · ${x.role}`;return `<label>${label}<select name="${name}" ${label.includes('*')?'required':''}><option value="">Не выбрано</option>${source.map(x=>`<option value="${opts==='employees'?escapeHtml(x.name):x.id}" ${(opts==='employees'?x.name:x.id)==value?'selected':''}>${escapeHtml(display(x))}</option>`).join('')}</select></label>`}
  ...
  }).join('');if(type==='catalog')$('#entityFields').insertAdjacentHTML('beforeend','<label>Валюта поставщика<input name="supplierCurrency" readonly></label><label>Условия поставщика<input name="supplierTerms" readonly></label>');$('#entityError').textContent='';bindEditorTools(type);entityDialog.showModal();
}
function bindCards(){
...
function openInventoryCount(){
  ...
  $('#entityFields').innerHTML=`<section class="inventory-count-workspace full"><div class="inventory-count-summary"><div><small>Позиций</small><b>${state.catalog.length}</b></div><div><small>Уже проверено</small><b>${Object.keys(session.counts).length}</b></div><label class="search">⌕ <input type="search" data-count-search placeholder="Найти товар или артикул"></label></div><div class="inventory-count-head"><span>Товар</span><span>По учёту</span><span>Фактически</span><span>Расхождение</span></div><div class="inventory-count-list">${state.catalog.map(x=>{const book=Number(x.stock)||0,actual=session.counts[x.id]??book;return `<label data-count-row data-book="${book}"><span><b>${escapeHtml(x.name)}</b><small>${escapeHtml(x.sku)} · ${escapeHtml(x.category||'Без категории')}</small></span><output>${book}</output><input type="number" min="0" step="1" name="count_${x.id}" value="${actual}" aria-label="Фактический остаток ${escapeHtml(x.name)}"><em data-count-diff>${Number(actual)-book}</em></label>`}).join('')}</div></section>`;
...
$('#profileButton').onclick=()=>{const p=state.profile,parts=String(p.name||'').trim().split(/\s+/);p.firstName=p.firstName||parts[0]||'';p.lastName=p.lastName||parts.slice(1).join(' ')||'';Object.entries(p).forEach(([k,v])=>{const el=$(`[name="${k}"]`,$('#profileForm'));if(el)el.value=v});avatarDraft=p.avatar;applyProfile();profileDialog.showModal()};
$$('[name="firstName"],[name="lastName"]',$('#profileForm')).forEach(input=>{input.addEventListener('beforeinput',event=>{if(event.data&&!/^\p{L}+$/u.test(event.data))event.preventDefault()});input.addEventListener('input',()=>{input.value=input.value.replace(/[^\p{L}]/gu,'')})});
...
$('#profileForm').onsubmit=e=>{e.preventDefault();const data=Object.fromEntries(new FormData(e.target)),word=/^\p{L}+$/u;data.firstName=data.firstName.trim();data.lastName=data.lastName.trim();if(!word.test(data.firstName)||!word.test(data.lastName)){notify('Имя и фамилия должны содержать по одному слову только из букв');return}data.name=`${data.firstName} ${data.lastName}`;state.profile={...state.profile,...data,avatar:avatarDraft};save('profile',state.profile);applyProfile();profileDialog.close();notify('Профиль обновлен')};
