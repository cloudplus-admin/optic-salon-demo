...
function syncDashboard(){
  ...
  $('#dailyRevenue').textContent=`${revenue.toLocaleString('ru')} сум`;$('#averageCheck').textContent=`${average.toLocaleString('ru')} сум`;$('#newClients').textContent=state.clients.length;
  $('#personalSales').textContent=`${revenue.toLocaleString('ru')} сум`;$('#personalPercent').textContent=`${percent}%`;$('#personalRing').textContent=`${percent}%`;$('#personalProgress').style.width=`${percent}%`;$('#personalRemaining').textContent=`${Math.max(plan-revenue,0).toLocaleString('ru')} сум`;
...
function showOrder(id){
  ...
  $('#detailContent').innerHTML=`<div class="detail-list">${[['Клиент',o.client],['Телефон',o.phone],['Статус',o.status],['Срок',o.deadline],['Сумма',`${Number(o.sum).toLocaleString('ru')} сум`],['Оплата',o.payment]].map(x=>`<div><small>${x[0]}</small><strong>${escapeHtml(x[1])}</strong></div>`).join('')}</div><div class="item-actions" style="margin-top:18px"><button data-order-status="В работе">В работу</button><button data-order-status="Готов">Готов</button><button data-order-status="Выдан">Выдать</button><button data-detail-sms>✉ SMS</button></div>`;
...
function itemCard(x,type){
  const map={
    ...
    catalog:[`${x.category} · ${x.brand||'Без бренда'}`,`Арт. ${x.sku}`,`${Number(x.price||0).toLocaleString('ru')} сум · остаток ${x.stock}`],
...
function productionBoard(){
  const active=state.orders.filter(o=>o.status!=='Выдан');
...
function workshopCard(o,index){
  const attention=o.status==='Требует обеспечения'||o.priority==='Срочный',s=statusView[o.status]||statusView['В работе'];
  ...
  return `<article class="workshop-card card ${attention?'needs-attention':''}" data-work-item><header><div class="workshop-order"><span class="tag">${escapeHtml(o.direction||'Оптика')}</span><b>№ ${o.id}</b><span class="status ${s[0]}">${escapeHtml(o.status)}</span></div><span class="priority ${attention?'high':''}">${escapeHtml(o.priority||'Обычный')}</span></header><div class="workshop-title"><div><h3>${escapeHtml(o.product||'Индивидуальный заказ')}</h3><p>${escapeHtml(o.client)} · ${escapeHtml(o.phone)}</p></div><div class="deadline"><small>Срок</small><b>${escapeHtml(o.deadline)}</b></div></div><div class="workshop-core"><span><small>Текущий этап</small><b>${escapeHtml(o.stage||o.status)}</b></span><span><small>Ответственный</small><b>${escapeHtml(o.responsible||'Не назначен')}</b></span><span><small>Филиал</small><b>${escapeHtml(branch)}</b></span><span><small>Оплата</small><b>${escapeHtml(o.payment)}</b></span></div><div class="workshop-progress"><div><span>${o.progress||40}% готовности</span><small>${attention?'Нужно действие сотрудника':'Работа идёт по плану'}</small></div><div class="mini-progress"><i style="width:${o.progress||40}%"></i></div></div><details><summary>Полная информация по работе <span>＋</span></summary><div class="workshop-details"><span><small>Обеспечение</small><b>${escapeHtml(supply)}</b></span><span><small>Рецепт / параметры</small><b>${escapeHtml(prescription)}</b></span><span><small>Контроль качества</small><b>${escapeHtml(o.quality||'ОТК назначен после текущего этапа')}</b></span><span><small>Стоимость заказа</small><b>${Number(o.sum).toLocaleString('ru')} сум</b></span></div><div class="workshop-history"><b>Последние события</b><p>Сегодня, 14:20 · ${escapeHtml(o.stage||o.status)}</p><p>Сегодня, 10:05 · Материалы и параметры проверены</p></div></details><footer><button class="secondary" data-demo-action="Карточка заказа №${o.id} открыта">Открыть заказ</button><button class="secondary" data-demo-action="Ответственный по заказу №${o.id} изменён">Назначить мастера</button><button class="primary" data-advance="${o.id}">${o.status==='Готов'?'Передать на выдачу':'Завершить этап'}</button></footer></article>`;
}
...
function bindCards(){
  ...
  $$('[data-demo-action]').forEach(b=>b.onclick=()=>notify(b.dataset.demoAction));
...
$('#entityForm').onsubmit=e=>{
  e.preventDefault();if(entityDialog.dataset.mode){const data=Object.fromEntries(new FormData(e.target));entityDialog.close();notify(`${entityDialog.dataset.mode==='refund'?'Возврат':'Оплата'} на ${Number(data.amount).toLocaleString('ru')} сум проведена`);entityDialog.dataset.mode='';return}
