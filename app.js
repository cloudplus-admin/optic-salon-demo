...
function settingsView(){
  ...
  return `<div class="settings-layout"><section class="card settings-card"><span class="settings-icon">◎</span><div><span class="eyebrow">Интерфейс</span><h2>Язык и оформление</h2><p>Изменения применяются сразу и сохраняются в браузере.</p></div><div class="settings-fields"><label>Язык интерфейса<select data-setting="language"><option value="ru" ${p.language==='ru'?'selected':''}>Русский</option><option value="uz" ${p.language==='uz'?'selected':''}>O‘zbekcha</option><option value="en" ${p.language==='en'?'selected':''}>English</option></select></label><label>Цветовая тема<select data-setting="theme"><option value="light" ${p.theme==='light'?'selected':''}>Светлая</option><option value="soft" ${p.theme==='soft'?'selected':''}>Мягкая голубая</option><option value="dark" ${p.theme==='dark'?'selected':''}>Тёмная</option></select></label></div><div class="theme-preview"><button data-theme-choice="light" title="Светлая"></button><button data-theme-choice="soft" title="Голубая"></button><button data-theme-choice="dark" title="Тёмная"></button></div></section><section class="card settings-card"><span class="settings-icon">◈</span><div><span class="eyebrow">Рабочее место</span><h2>Поведение системы</h2><p>Настройте уведомления и безопасность демо.</p></div><div class="settings-switches">${['Двухфакторная аутентификация','Уведомления о низких остатках','SMS при готовности заказа','Автоматическое резервное копирование'].map((x,i)=>`<label class="switch">${x}<input type="checkbox" ${i!==0?'checked':''}></label>`).join('')}</div></section></div><button class="primary" data-save-settings style="margin-top:18px">Сохранить настройки</button>`;
}
...
function applyPreferences(){
  const p=state.preferences||{language:'ru',theme:'light'};if(p.theme==='warm')p.theme='soft';document.documentElement.dataset.theme=p.theme;document.documentElement.lang=p.language;
